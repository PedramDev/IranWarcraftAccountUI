...
import { ProductViewModel , GetProductList } from 'src/app/models/shop/products';
...
export class AppProductListComponent implements OnInit {
    ...
    options : GetProductList = new GetProductList();    
    checked = false;
    indeterminate = false;
    listOfCurrentPageData: readonly ProductViewModel[] = [];
    listOfData: readonly ProductViewModel[] = [];
    ...
    onCurrentPageDataChange($event: readonly ProductViewModel[]): void {
    ...
    load(options : GetProductList | null){
      this.isLoading=true;
    ...
    isRowDeleting(item:ProductViewModel){
    ...
    toggleEnabled(item:ProductViewModel){
    ...
    toggleEnabledLoading(item:ProductViewModel){
    ...
    create(){
      this.service.add().subscribe({
        next: (v) => {
          this.router.navigate(['/cpanel/product/'+v.data], { relativeTo: this.route });
        },
        error: (e) => {
          console.error(e)
